...
<?php else : ?>
<div class="post-listing news_posts_cat">
<?php while ( have_posts() ) : the_post(); ?>
<?php if( tie_get_option( 'blog_display' ) != 'full_thumb' ): ?>
	<article <?php tie_post_class('item-list'); ?>>
		<!--<h2 class="post-box-title titulo_posts"><a href="<?php the_permalink(); ?>" title="<?php //printf( esc_attr__( 'Permalink to %s', 'tie' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2> -->
		<?php //get_template_part( 'includes/archives-meta' ); ?>					

		<?php if( tie_get_option( 'blog_display' ) == 'content' ): ?>
		<div class="entry entradas-1">
			<?php the_content( __( 'Read More &raquo;', 'tie' ) ); ?>
		</div>
		<?php else: ?>
			<?php if ( function_exists("has_post_thumbnail") && has_post_thumbnail() ) : ?>			
		<div class="post-thumbnail imagen_post">
			...
			<?php endif; ?>
		<h2 class="post-box-title titulo_posts"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'tie' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
		<div class="entry entradas-2">
			<p><?php tie_excerpt() ?>
				<a class="more-link aaaa" href="<?php the_permalink() ?>"><?php //_e( 'Read More &raquo;', 'tie' ) ?>
					<img src="<?php echo home_url(); ?>/wp-content/themes/planH/images/mas.png" alt="leer mas">
				</a>
			</p>
		</div>
		<?php endif; ?>
